...
export default function App() {
  ...
  function createRequest({date, shift, reason, instructorId, instructorName, imageUrl}) {
    const r = {
      id: uid('r'), 
      userId: user.id, 
      userName: user.name, 
      ...
      {visible.map(r => (
        <div key={r.id} className="request">
          ...
          <div style={{display: 'flex', flexDirection: 'column', gap: '8px', alignItems: 'flex-end'}}>
            {r.status === 'pending' && currentUser && (currentUser.role === 'admin' || currentUser.role === 'instructor') && (
              <>
